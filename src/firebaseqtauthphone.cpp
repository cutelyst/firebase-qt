...
{
public:
    FirebaseQtAuthPhoneListener(FirebaseQtAuthPhone *q)
        : q_ptr(q)
    {
    }
...
{
public:
    FirebaseQtAuthPhonePrivate(FirebaseQtAuth *a)
        : auth(a)
    {
    }
...
FirebaseQtAuthCredential FirebaseQtAuthPhone::getCredential(const QString &verificationId,
                                                            const QString &code) const
{
    Q_D(const FirebaseQtAuthPhone);
    auth::PhoneAuthProvider &phone_provider =
        auth::PhoneAuthProvider::GetInstance(d->auth->d_ptr->auth);
    auth::Credential cred = phone_provider.GetCredential(verificationId.toLatin1().constData(),
    ...
    firebase::auth::PhoneAuthCredential credential)
{
    ...
    qCDebug(FIREBASE_AUTHPHONE) << "OnVerificationCompleted" << QThread::currentThreadId()
...
void FirebaseQtAuthPhoneListener::OnVerificationFailed(const std::string &error)
{
    ...
    qCDebug(FIREBASE_AUTHPHONE) << "OnVerificationFailed" << QThread::currentThreadId()
                                << QString::fromStdString(error);

    QMetaObject::invokeMethod(
        q_ptr, &FirebaseQtAuthPhone::verificationFailed, QString::fromStdString(error));
}
    ...
    const firebase::auth::PhoneAuthProvider::ForceResendingToken &force_resending_token)
{
    ...
    QMetaObject::invokeMethod(
        q_ptr, &FirebaseQtAuthPhone::codeSent, QString::fromStdString(verification_id));
}
