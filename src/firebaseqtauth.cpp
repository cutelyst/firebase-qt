...
void FirebaseQtAuth::signIn(const FirebaseQtAuthCredential &credential)
{
    ...
    future.OnCompletion([=](const Future<auth::User> &result) {
        if (result.error()) {
            qWarning() << "signIn future error" << result.error() << result.error_message()
                       << result.status() << result.result();

            QMetaObject::invokeMethod(this,
                                      &FirebaseQtAuth::signInError,
                                      result.error(),
            ...
            f2.OnCompletion([=](const Future<std::string> &r2) {
                if (result.error()) {
                    qWarning() << "get token future error" << result.error() << r2.error_message()
                               << r2.status() << r2.result();
                    ...
                    QMetaObject::invokeMethod(
                        this, &FirebaseQtAuth::signInToken, QString::fromStdString(token));
                }
            });
        }
    });
