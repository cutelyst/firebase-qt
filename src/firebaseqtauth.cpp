...
FirebaseQtAuth::FirebaseQtAuth(FirebaseQtApp *app)
    : FirebaseQtAbstractModule(app), d_ptr(new FirebaseQtAuthPrivate) {}
...
void FirebaseQtAuth::initialize(FirebaseQtApp *app) {
  ...
  Q_ASSERT_X(result == InitResult::kInitResultSuccess,
             "FirebaseQtAuth::initialize()", "result");
...
void FirebaseQtAuth::signIn(const FirebaseQtAuthCredential &credential) {
  ...
  Future<auth::User> future =
      d->auth->SignInWithCredential(credential.d->credential);
  future.OnCompletion([=](const Future<auth::User> &result) {
    if (result.error()) {
      qWarning() << "signIn future error" << result.error()
                 << result.error_message() << result.status()
                 << result.result();

      QMetaObject::invokeMethod(this, &FirebaseQtAuth::signInError,
                                result.error(),
      ...
      f2.OnCompletion([=](const Future<std::string> &r2) {
        if (result.error()) {
          qWarning() << "get token future error" << result.error()
                     << r2.error_message() << r2.status() << r2.result();

          QMetaObject::invokeMethod(this, &FirebaseQtAuth::signInError,
                                    result.error(),
                                    QString::fromStdString(r2.error_message()));
        } else {
          std::string token = *r2.result();
          qDebug() << "get token future" << user.uid().c_str() << token.c_str();

          QMetaObject::invokeMethod(this, &FirebaseQtAuth::signInToken,
