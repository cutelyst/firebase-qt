...
class FirebaseQtMessagingPrivate final
...
FirebaseQtMessaging::~FirebaseQtMessaging() { delete d_ptr; }
...
void FirebaseQtMessagingPrivate::OnMessage(
    const firebase::messaging::Message &message) {
  qDebug(FIREBASE_MESSAGING)
      << "OnMessage" << QString::fromStdString(message.from)
      << QString::fromStdString(message.notification->title);

  QMetaObject::invokeMethod(q_ptr, &FirebaseQtMessaging::messageReceived,
                            message);
}
...
void FirebaseQtMessagingPrivate::OnTokenReceived(const char *token) {
  ...
  QMetaObject::invokeMethod(q_ptr, &FirebaseQtMessaging::tokenReceived,
                            QByteArray{token});
}
